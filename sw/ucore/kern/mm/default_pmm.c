...
basic_check(void) {
    ...
    // 2026-final: chiplab/nscscc adaptation: managed pages start at KMEMBASE
    assert(page2pa(p0) < KMEMBASE + npage * PGSIZE);
    assert(page2pa(p1) < KMEMBASE + npage * PGSIZE);
    assert(page2pa(p2) < KMEMBASE + npage * PGSIZE);
