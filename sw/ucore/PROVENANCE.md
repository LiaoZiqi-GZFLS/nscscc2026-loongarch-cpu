...
| `kern/driver/console.c` | UART init = chiplab sequence: FCR=0x47, DLL=1/DLM=0, 8N1 | matches chiplab start.S for the exact URT IP at phys 0x1fe001e0 |
| `kern/include/loongarch.h` | `COM1_BAUD_DDL` 0x23 -> 0x1; `CACHELINE_SIZE` 16 -> 64 | divisor per chiplab start.S; our NOP core has 64-byte I/D cache lines (MyCPUConfig.scala), fence_i() CACOP stride must match |
